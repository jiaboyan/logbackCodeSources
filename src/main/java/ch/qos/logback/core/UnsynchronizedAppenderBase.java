...
abstract public class UnsynchronizedAppenderBase<E> extends ContextAwareBase implements Appender<E> {
    ...
    public void doAppend(E eventObject) {
        //guard：线程本地变量；从该对象中获取存储的值，看是否为true;
        if (Boolean.TRUE.equals(guard.get())) {
            return;
        }
        try {
            //如果不为true，则向guard中设置一个线程本地变量为true的值；
            ...
            //获取绑定在Appender上的 过滤器，如果存在过滤器就进行调用：
            //FilterReply包含了：“DENY(拒绝日志记录请求), NEUTRAL(通过日志记录请求)和ACCEPT(继续调用下一个filter)”
            if (getFilterChainDecision(eventObject) == FilterReply.DENY) {
                return;
            }
            //RollingFileAppender ConsoleAppender都会走OutputStreamAppender类的append方法：
