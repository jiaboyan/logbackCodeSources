...
public class OutputStreamAppender<E> extends UnsynchronizedAppenderBase<E> {
    ...
    //RollingFileAppender ConsoleAppender都会走这个方法：
    @Override
    protected void append(E eventObject) {
        if (!isStarted()) {
            return;
        }
        //RollingFileAppender对象会走RollingFileAppender自身的subAppend方法：
        //ConsoleAppender对象会直接OutputStreamAppender类的subAppend方法
    ...
    protected void writeOut(E event) throws IOException {
        //这个encoder就是我们对应Appender中设置的"日志输出格式":
        //ConsoleAppender..RollingFileAppender都调用的是LayoutWrappingEncoder类的doEncode方法：
    ...
    protected void subAppend(E event) {
        ...
        try {
            ...
            lock.lock();
            try {
                //调用日志打印方法：如果是RollingFileAppender类则调用FileAppender的writeOut方法：
                //如果是ConsoleAppender类则直接调用OutputStreamAppender的writeOut方法：
                writeOut(event);
            } finally {
                //释放锁：
                lock.unlock();
