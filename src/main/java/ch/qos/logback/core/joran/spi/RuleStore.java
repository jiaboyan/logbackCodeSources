 ...
 * 
 */
// ruleStore是用来根据Sax节点（new ElementSelector("configuration")），来匹配对应的Action。
// ruleStore是在Interpreter创建时初始化的，而且只初始化一次，
// 因此logback.xml中的每个元素，用哪个Action来处理，都已经规定好了。
// 如果在解析logback.xml的过程中，遇到了无法识别的元素，就会抛出异常
